...
public class CameraView extends FrameLayout {
    ...
    private void init(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        mCameraController.setAfterStartingRecordRunnable(new Runnable() {
            @Override
            public void run() {
                for (final CameraListener cameraListener : mListeners) {
                    if (cameraListener != null) post(new Runnable() {
                        @Override
                        public void run() {
                            cameraListener.onStartRecordVideo();
                        }
                    }) ;
                }
            }
        });
        mCameraController.setBeforeStopingRecordRunnable(new Runnable() {
            @Override
            public void run() {
                for (final CameraListener cameraListener : mListeners) {
                    if (cameraListener != null) post(new Runnable() {
                        @Override
                        public void run() {
                            cameraListener.onStopRecordVideo();
                        }
                    }) ;
                }
            }
